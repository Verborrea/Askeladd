<script>
	...
	export let data, type, name, status, isClean = () => true

	let saveExamLoading = false

	const saveExam = ({ cancel }) => {
		...
		return async ({ update }) => {update(); saveExamLoading = false;}
	}
</script>
...
<header>
	...
	<div class="flex">
		{#if data}
			<form action="?/saveExam" method="post" id="saveExam" use:enhance={saveExam}>
				<button type="submit" class="button red" disabled={saveExamLoading}>
